...
if [[ ( -z ${1} || "-h" = "${1}" ) || ( "--help" = "${1}" ) ]]; then
  ...
  echo '|                                                                   |'
  echo '| docker run -itd \                                                 |'
  echo '|            --name postgresql -itd --restart always \              |'
  echo '|            --publish 5432:5432 \                                  |'
  echo '|            --volume /srv/docker/postgresql:/var/lib/postgresql \  |'
  echo '|            rgomes/postgresql start [args...]                      |'
  echo '|                                                                   |'
  echo '| Login to the PostgreSQL server using:                             |'
  echo '|                                                                   |'
  echo '|     docker exec -it postgresql sudo -u postgres psql              |'
  echo '+-------------------------------------------------------------------+'
elif [[ ${1} = start ]]; then
    # launch postgress and pass arguments
    EXTRA_ARGS="${@:2}"
    set --
